...
** Python
The integral is a bit nasty because of the oscillating behaviour of the Bessel funtion $J_0$ so we need to adjust the =scipy.integrate= settings a little bit to reach a desired accuracy.
...
We see that due to the larger colour charge in the gluon-induced Higgs process ($C_A = 3 > \tfrac{4}{3} = C_F$), the localtion of the Sudakov peak is further to the right.
